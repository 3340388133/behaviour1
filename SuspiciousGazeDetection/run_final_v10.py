#!/usr/bin/env python3
"""
可疑行为检测系统 V10 - 极严格阈值版 (目标: 10-20%可疑率)

关键改动：
1. prolonged_watch: yaw>55° → 60°, 持续5秒
2. glancing: yaw范围>60°, 转头≥5次
3. 加入"持续性"要求：同一行为需要触发多次才标记

行为分类：
0 - normal:          正常行为
1 - glancing:        频繁张望，3秒内左右转头≥5次，yaw变化>60°
2 - quick_turn:      快速回头，0.5秒内yaw变化>80°
3 - prolonged_watch: 长时间观察，持续>5秒注视非正前方(yaw>60°)
4 - looking_down:    持续低头，pitch<-30° 持续>6秒
5 - looking_up:      持续抬头，pitch>30° 持续>4秒
"""
...
class Config:
    ...
    GLANCING_MIN_TURNS = 5         # 左右转头≥5次 (V9是4)
    GLANCING_YAW_CHANGE = 60.0     # yaw变化>60° (V9是50)

    # 2. quick_turn: 快速回头 (极严格)
    QUICK_TURN_WINDOW = 15         # 0.5秒窗口
    QUICK_TURN_YAW_CHANGE = 80.0   # yaw变化>80° (V9是70)

    # 3. prolonged_watch: 长时间观察 (极严格)
    PROLONGED_WATCH_DURATION = 150 # 持续>5秒 (V9是4秒)
    PROLONGED_WATCH_YAW = 60.0     # yaw>60° (V9是45)

    # 4. looking_down: 持续低头 (极严格)
    LOOKING_DOWN_DURATION = 180    # 持续>6秒 (V9是5秒)
    LOOKING_DOWN_PITCH = -30.0     # pitch<-30° (V9是-25)

    # 5. looking_up: 持续抬头 (极严格)
    LOOKING_UP_DURATION = 120      # 持续>4秒 (V9是3秒)
    LOOKING_UP_PITCH = 30.0        # pitch>30° (V9是25)

    # 持续性要求：同一行为需要触发N次才真正标记为可疑
    BEHAVIOR_CONFIRM_COUNT = 2     # 需要触发2次才确认

    MIN_TRACK_FRAMES = 45          # 最少跟踪1.5秒才分析
...
class BehaviorAnalyzer:
    ...
    def detect_prolonged_watch(self, yaw_history):
        ...
        is_prolonged = ratio > 0.90  # 提高到90%以上时间偏离
...
class SuspiciousDetectorV10:
    def __init__(self):
        ...
        print("=" * 60)
        print("  可疑行为检测系统 V10 (极严格阈值, 目标10-20%)")
        print("=" * 60)
        print("行为判定标准 (V10 极严格):")
        ...
        print(f"  4. look_down: pitch<{self.cfg.LOOKING_DOWN_PITCH}°, 持续>6秒")
        print(f"  5. look_up: pitch>{self.cfg.LOOKING_UP_PITCH}°, 持续>4秒")
        print(f"  确认要求: 同一行为需触发≥{self.cfg.BEHAVIOR_CONFIRM_COUNT}次")

        os.makedirs(self.cfg.OUTPUT_DIR, exist_ok=True)
        os.makedirs(os.path.join(self.cfg.OUTPUT_DIR, "samples_v10"), exist_ok=True)
    ...
    def process_video(self, video_name, camera_type, yaw_offset):
        ...
        output_path = os.path.join(self.cfg.OUTPUT_DIR, f"{video_name}_v10.mp4")
        ...
        # V10新增: 行为确认计数
        track_behavior_trigger_count = defaultdict(lambda: defaultdict(int))
        ...
        while frame_idx < max_frames:
            ...
            if results[0].boxes.id is not None:
                ...
                for box, tid in zip(boxes, track_ids):
                    ...
                    if frame_idx % 15 == 0 and len(track_yaws[tid]) >= self.cfg.MIN_TRACK_FRAMES:
                        ...
                        # V10: 行为确认机制 - 需要多次触发才算真正可疑
                        confirmed_behaviors = []
                        for b in analysis['behaviors']:
                            track_behavior_trigger_count[tid][b] += 1
                            if track_behavior_trigger_count[tid][b] >= self.cfg.BEHAVIOR_CONFIRM_COUNT:
                                confirmed_behaviors.append(b)

                        analysis['behaviors'] = confirmed_behaviors
                        analysis['is_suspicious'] = len(confirmed_behaviors) > 0

                        if analysis['is_suspicious']:
                            # 更新颜色
                            priority = [self.analyzer.QUICK_TURN, self.analyzer.GLANCING,
                                       self.analyzer.PROLONGED_WATCH, self.analyzer.LOOKING_DOWN,
                                       self.analyzer.LOOKING_UP]
                            for b in priority:
                                if b in confirmed_behaviors:
                                    analysis['primary_behavior'] = b
                                    analysis['color'] = self.analyzer.BEHAVIOR_COLORS[b]
                                    break
            ...
            cv2.putText(annotated, f"V10 Frame {frame_idx} | Normal:{normal_count} Susp:{suspicious_count} ({ratio:.0f}%)",
            ...
            if frame_idx in sample_frames:
                cv2.imwrite(os.path.join(self.cfg.OUTPUT_DIR, "samples_v10", f"{video_name}_{frame_idx:05d}.jpg"), annotated)
