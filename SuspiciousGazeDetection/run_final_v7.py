#!/usr/bin/env python3
"""
可疑行为检测系统 V7 - 降低阈值，更敏感
检测模式：
1. 频繁张望 (Scanning) - 头部左右转动
2. 固定凝视 (Fixation) - 长时间盯着非正前方
3. 快速扫视 (Quick Glance) - 突然转头
...
class Config:
    ...
    # 模式1: 频繁张望 (Scanning) - 降低阈值
    SCANNING_YAW_RANGE = 40.0        # yaw范围>40度 (原80)
    SCANNING_DIR_CHANGES = 2         # 方向变化>2次 (原4)

    # 模式2: 固定凝视 (Fixation) - 降低阈值
    FIXATION_YAW_THRESHOLD = 25.0    # 偏离正前方>25度 (原35)
    FIXATION_DURATION = 45           # 持续1.5秒 (原2秒)
    FIXATION_STABILITY = 20.0        # yaw变化<20度算稳定 (原15)

    # 模式3: 快速扫视 (Quick Glance) - 大幅降低
    QUICK_GLANCE_SPEED = 8.0         # 每帧yaw变化>8度 (原25)
    QUICK_GLANCE_COUNT = 1           # 出现>1次 (原2)

    # 模式4: 徘徊 (Loitering)
    LOITER_DURATION = 120            # 停留>4秒 (原5秒)
    LOITER_MOVE_THRESHOLD = 120      # 移动距离<120像素 (原100)

    # 综合评分阈值
    SUSPICIOUS_THRESHOLD = 30        # 评分>=30判定可疑 (原40)

    MIN_TRACK_FRAMES = 20            # 最少跟踪帧数 (原30)
...
class MultiPatternAnalyzer:
    ...
    def detect_scanning(self, yaw_history):
        ...
        dir_changes = 0
        prev_direction = None
        for i in range(5, len(yaws)):
            # 比较当前位置和5帧前的位置
            diff = yaws[i] - yaws[i-5]
            if abs(diff) > 5:  # 忽略小幅抖动
    ...
    def detect_quick_glance(self, yaw_history):
        ...
        recent = list(yaw_history)[-60:]  # 最近2秒
        quick_count = 0
        max_speed = 0
    ...
    def analyze(self, yaw_history, position_history):
        ...
        results['suspicious'] = score >= self.cfg.SUSPICIOUS_THRESHOLD

        return results


class SuspiciousDetectorV7:
    def __init__(self):
        self.cfg = Config()
        ...
        print(f"  可疑阈值: 评分>={self.cfg.SUSPICIOUS_THRESHOLD}")
        print(f"输出目录: {self.cfg.OUTPUT_DIR}")

        os.makedirs(self.cfg.OUTPUT_DIR, exist_ok=True)
        os.makedirs(os.path.join(self.cfg.OUTPUT_DIR, "samples_v7"), exist_ok=True)
    ...
    def process_video(self, video_name, camera_type, yaw_offset):
        ...
        output_path = os.path.join(self.cfg.OUTPUT_DIR, f"{video_name}_v7.mp4")
        ...
        while frame_idx < max_frames:
            ...
            cv2.putText(annotated, f"V7 Frame {frame_idx} | Normal:{green_count} Suspicious:{red_count}",
            ...
            if frame_idx in sample_frames:
                sample_path = os.path.join(self.cfg.OUTPUT_DIR, "samples_v7", f"{video_name}_frame_{frame_idx:05d}.jpg")
